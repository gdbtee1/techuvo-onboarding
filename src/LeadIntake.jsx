  ...
  const handleSubmit = () => {
    ...
    if (missing) {
      ...
      setSubmissionMessage(
        `Please add ${missing[1]} before submitting your website request.`,
      );
      return;
    }

    const subject = `NEW TECHUVO WEBSITE LEAD — ${form.businessName}`;

    const body = `NEW TECHUVO WEBSITE REQUEST
...
City / service area: ${form.city || "Not provided"}
Existing website: ${form.existingWebsite || "None provided"}
...
Pages requested:
${form.pages || "Not provided"}

Special features:
${form.features || "Not provided"}

Ideal customer:
${form.targetCustomer || "Not provided"}

STYLE DIRECTION
Preferred colors:
${form.colors || "Not provided"}

Overall style:
${form.style || "Not provided"}

Websites / inspiration:
${form.inspiration || "Not provided"}

Things to avoid:
${form.avoid || "Not provided"}

FILES / ASSET LINKS
${form.assetLinks || "No asset links provided. Client can attach files directly to this email before sending."}

FINAL NOTES
${form.notes || "None"}

---
Submitted through the Techuvo website request form.
`;

    const mailtoUrl = `mailto:techuvodesign@gmail.com?subject=${encodeURIComponent(
      subject,
    )}&body=${encodeURIComponent(body)}`;

    setFormStatus("success");
    setSubmissionMessage(
      "Your email app is opening with your Techuvo website request already filled in. Attach any files you want us to receive, then press Send.",
    );
  ...
  const smsHref = `sms:+13134507265?body=${encodeURIComponent(
    `Hi Techuvo, I just submitted my website request. Business: ${
                ...
                {step === 2 && (
                  <div>
                    ...
                    <h2 className="mt-6 max-w-4xl text-[clamp(3.2rem,7vw,6.5rem)] font-black leading-[0.86] tracking-[-0.07em]">
                      Send us the
                      <span className="block text-blue-600">
                        assets we should use.
                      </span>
                    </h2>

                    <p className="mt-6 max-w-2xl text-lg font-semibold leading-8 text-slate-600">
                      Paste Google Drive, Dropbox, iCloud, OneDrive, YouTube, or
                      other share links below. You can also attach files directly
                      to the email that opens when you submit.
                    </p>

                    <div className="mt-10 rounded-[2rem] border-[3px] border-slate-950 bg-[#ffd3c8] p-5 shadow-[8px_9px_0_#0f172a] sm:p-7">
                      <TextArea
                        label="File / asset links"
                        name="assetLinks"
                    ...
                    <div className="mt-6 rounded-[1.5rem] border-[3px] border-slate-950 bg-white p-5 shadow-[5px_6px_0_#0f172a]">
                      <p className="font-black">
                        Have files saved directly on your phone or computer?
                      </p>

                      <p className="mt-2 text-sm font-semibold leading-6 text-slate-500">
                        When you finish this form, your email app will open with
                        your request already filled in. Attach your logos, photos,
                        PDFs, videos, or other files there before pressing Send.
                      </p>
                    </div>
                  </div>
                )}
